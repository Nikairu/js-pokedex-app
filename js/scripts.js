let pokemonRepository = (function () {
  ...
  function add(item) {
    if (
      ...
      Object.prototype.hasOwnProperty.call(item, 'types')
    ) {
  ...
  function addListItem(pokemon) {
    let newElement = $(
      `<button class="pokemon-button" data-toggle="modal" data-target="#exampleModal">${pokemon.name}</button>`
    );
  ...
  function showModal(pokemon) {
    $('.modal-title').html(
      `<h1 style='color:${pokeColour(pokemon.types[0].type.name)};'>${
        pokemon.name
      }</h1>`
    );
    ...
    $('.modal-body').html('');
    $('.modal-body').append(
      `<img src='${pokemon.imageUrl}' class='pokemon-sprite' ></img>`
    );
